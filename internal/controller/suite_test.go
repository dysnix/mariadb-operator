...
var _ = BeforeSuite(func() {
	...
	var err error
	testCidrPrefix, err = docker.GetKindCidrPrefix(testCtx)
	...
	err = NewUserReconciler(client, refResolver, secretReconciler, conditionReady, sqlOpts...).SetupWithManager(testCtx, k8sManager)
	Expect(err).ToNot(HaveOccurred())
